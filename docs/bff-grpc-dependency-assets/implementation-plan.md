...
operation 作为 impact source，可与 `--diff` 组合为同一份 impact JSON。独立 gRPC 反向查询命令
已移除；下方实施记录统一以当前 `impact --grpc` 契约为准。
...
- `internal/app/dependency.go`：`RunEndpointAssets*` 严格 pipeline。
...
不得给 `SchemaJSON` 增加 `endpoint-assets` 类型。
...
reverse := dependency.FindGrpcImpactSources(store, grpcMethods)
...
endpoint identity 解析规则：优先使用 `RouteGraph.AnnotationsForHandler` 的 method/path；没有 annotation 时使用已解析 route method/path。已解析 route 同时作为同级 `routes` 辅助证据输出，但不改变 annotation endpoint 的正式 identity。只有被 route 注册证明的 handler 才能成为 endpoint。
...
- `impact --grpc` 的 source identity 直接从合法 canonical 输入拆分，因此 catalog 中不存在、当前 BFF 未消费的 gRPC 仍能返回空 consumers。
...
### Task 9: 实现 endpoint asset 与 impact gRPC source JSON contract
...
测试从 Task 8 的查询结果构建 endpoint asset document，并断言顶层键严格为：
...
Run: `go test ./internal/output -run 'TestEndpointAssets|TestAddGrpcSources' -count=1`
...
Run: `UPDATE_GOLDEN=1 go test ./internal/output -run 'TestEndpointAssetsGolden|TestAddGrpcSources'`
...
```bash
git add internal/output/dependency.go internal/output/endpoint_assets.go internal/output/grpc_impact.go internal/output/dependency_test.go testdata/golden/endpoint-assets.json
...
测试 `RunEndpointAssetsWithMetrics` 与带 `--grpc` 的 `RunImpactWithMetrics` 使用 strict mode，先完成所有输入与分析校验，再调用 render；任何错误时 `RunResult.Output` 必须为空。
...
- `help endpoint-assets` / `help impact`。
...
所有 endpoint 先统一 parse；任一不存在时在 render 前失败。`impact --grpc` 对合法但未被消费的 full method 正常返回空 consumers。metrics 顺序固定。
...
`TestGrpcDependenciesFactsGolden` 必须通过 `app.RunFacts`，而不是手工拼 Store；`TestEndpointAssetsGolden` 和 `TestAddGrpcSources` 必须通过 app entry。归一化仅允许 project root/build context，不得删除 call-site、handler、client 或 chain 字段。

- [ ] **Step 2: 运行测试确认 golden 缺失或不匹配**

Run: `go test ./internal/output -run 'TestGrpcDependencies|TestEndpointAssetsGolden|TestAddGrpcSources' -count=1`

Expected: FAIL，提示 golden 缺失或 mismatch。

- [ ] **Step 3: 生成 golden 并人工检查关键关系**

Run: `UPDATE_GOLDEN=1 go test ./internal/output -run 'TestGrpcDependencies|TestEndpointAssetsGolden|TestAddGrpcSources' -count=1`
...
Run: `go test ./internal/output -run 'TestGrpcDependencies|TestEndpointAssetsGolden|TestAddGrpcSources' -count=2`
...
```bash
git add internal/output/golden_test.go internal/output/grpc_impact_test.go testdata/golden/grpc-dependencies.facts.json testdata/golden/endpoint-assets.json testdata/fixtures/grpc-dependencies testdata/fixtures/grpc-proto
...
对三个项目运行 facts，并针对 baseline `cases` 分别执行 `endpoint-assets` 和 `impact --grpc`。脚本必须检查：
...
Run: `/tmp/go-analyzer help impact`

Expected: help 列出 endpoint 与 repeatable gRPC 输入和公共 build context 参数。
...
- `endpoint-assets` 支持多个 endpoint，`impact --grpc` 支持多个 canonical gRPC method。
