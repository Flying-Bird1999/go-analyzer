...
- 输出结构化调用链和源码位置，支持人工 review 和后续 Nexus 文档消费。
...
排序规则和本文档共同保证。Nexus 真正接入并需要机器校验时再补 schema。
...
错误必须携带稳定 code，不能要求 Nexus 或其他调用方解析自然语言 message：
...
本方案不修改 Nexus。Nexus 当前仍从 controller annotation 和 proto 生成 BFF/gRPC 文档。

未来集成使用两个稳定契约：

1. canonical full method：Nexus 可以从 proto descriptor 生成同一个 operation ID。
2. `endpoint-assets` 公开 JSON：Nexus 可以据此补充 gRPC 依赖、源码链接和依赖图。

Nexus 不应读取 analyzer 内部 facts、module cache 路径或 Go 内部包，也不引入两个工具之间
的 Go package 编译依赖。

## 19. 测试矩阵

### 19.1 Generated Catalog
...
3. 真实 BFF 中 direct、wrapper 和 Nexus getter 调用不依赖命名猜测即可识别。
...
8. Nexus 后续可以只依赖 canonical full method 和公开 asset JSON 完成文档集成。
