...
早期版本给每条传播关系挂一个 `high/medium/low` 置信度（`facts.CombineConfidence` 链路最弱合并），已整体移除（含 `facts.Confidence` 类型、`ReferenceFact`/`ChangeFact`/`LinkFact`/`GrpcProviderFact`/`DubboProviderFact`/`JobRegistrationFact`/`IMEventFact`/`ModuleUsageFact` 等字段与 JSON Schema 的 `confidence` 属性）。设计立场是：分析器只报能静态证明的关系，证明不了就降级为诊断或“未解析”标记（`im_event_unresolved`、诊断码等），不用一个“打折的把握度”去表达不确定性。

`internal/astindex` 内部仍保留一个私有布尔信号（`ValueType.Resolved`），仅用于三处准入门槛（map 值类型推断、interface 变量收窄、constructor 返回类型收窄的严格证据判断）——这是纯内部实现细节，不对外暴露，也不是分级置信度的替代品。

真实实现位置：`internal/output/grpc_service_impact.go`、`internal/output/contract.go`、`internal/astindex/index.go`、`internal/astindex/interface_bindings.go`。
...
| `internal/impact`、`internal/serviceimpact` | changed root 可达性、终点过滤、liveness、terminal relation |
