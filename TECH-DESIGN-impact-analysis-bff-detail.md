...
| 完整 gRPC 接口名 | `/proto包名.服务名/方法名` 形式的跨仓库唯一接口身份，来自 protobuf 生成代码，不是 Go 方法名        |
...
继续用第 2 节那条链路。`ListWinner` 最终调到的 gRPC，在 protobuf 生成代码里长这样：

```go
// Code generated by protoc-gen-go-grpc. DO NOT EDIT.
// gen/activity_user_api/activity_user_grpc.pb.go

const ActivityUserService_ListWinnerBySalesId_FullMethodName =
	"/gopkg.inshopline.com.sc1.app.modules.medium.activity_user.proto.ActivityUserService/listWinnerBySalesId"

func (c *activityUserServiceClient) ListWinnerBySalesId(
	ctx context.Context,
	in *ListWinnerBySalesIdReq,
	opts ...grpc.CallOption,
) (*PageActivityWinnerWithAvatarResp, error) {
	out := new(PageActivityWinnerWithAvatarResp)
	err := c.cc.Invoke(ctx, ActivityUserService_ListWinnerBySalesId_FullMethodName, in, out, opts...)
	return out, err
}
```
...
```text
Go 侧：ActivityUserServiceClient.ListWinnerBySalesId
                    ↓
完整接口名：/gopkg.inshopline.com.sc1.app.modules.medium.activity_user.proto.ActivityUserService/listWinnerBySalesId
```

建立这条对照需要同时看到：生成代码标记、`Invoke`（流式接口是 `NewStream`）调用、被传入的完整接口名常量、Client 接口声明、具体实现方法，以及构造函数把接口和实现绑定起来的那处代码。

**这个例子恰好说明为什么不能按 Go 方法名去猜接口名**，三处都对不上：

| 对不上的地方 | Go 侧                    | proto 侧                                                             |
| ------------ | ------------------------ | -------------------------------------------------------------------- |
| 方法名首字母 | `ListWinnerBySalesId`  | `listWinnerBySalesId`（小写开头）                                  |
...
也就是说，接口名里的 proto 包路径根本无法从 Go 的目录名或变量名推导出来，只能从生成代码里的那个字符串常量读。这个完整接口名是跨仓库串联的唯一稳定身份。
...
只有同时满足以下三条，才记录一次 BFF 到 gRPC 的调用关系：

1. 对照表里存在这个 Client 方法及其完整接口名。
2. 调用接收者（本例是包级变量 `live.ActivityUserClient`）的静态类型能唯一解析到对应的生成 Client。
3. 调用发生在本项目的函数或方法内部。

不会依据变量名、目录名、请求消息名或相似的方法名去猜接口。例如仅仅因为某处也叫 `ListWinnerBySalesId`、或者也用到了 `ListWinnerBySalesIdReq` 这个消息类型，都不足以认定它发起了这次 RPC。
...
GET /admin/api/bff-web/post/sale/:salesId/activity/winners
  ...
  -> /gopkg.inshopline.com...activity_user.proto.ActivityUserService/listWinnerBySalesId
```
...
| 完整 gRPC 接口名         | 完整 gRPC 接口名   | `/protobuf-package.Service/Method` 形式的接口身份                           |
| Generated Client Catalog | 生成客户端对照表   | Go Client Method 到完整 gRPC 接口名的对照表                                   |
