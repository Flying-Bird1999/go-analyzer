...
- **第三段 · 分链路出结果**：从事实仓库开始，按项目是 BFF 还是后端服务，走各自的传播 / 投影链路，产出稳定 JSON（这一段才是两类项目真正分开的地方，前两段是共用的）。
...
flowchart TB
    ...
    STORE[("facts.Store")]
    GRAPH["③ graph 反向引用图 + RouteGraph<br/>（route 域查询，共用）"]
...
| `graph`             | 用引用事实建反向引用图 + RouteGraph（route 域查询），并保证遍历顺序稳定（结果可回归）           | 传播图（两条链路都从它遍历）   |
...
| （BFF 专用抽取器）    | `annotation` / `im` / `grpc`(client) / `dependency`；传播走 `impact`               | 见 §3                         |
...
- 输出：一份 JSON，结构和 BFF **同构**——`summary`（总览）+ `fileSources[]`（每个变更文件的完整传播树）+ `entrySourcesSummary`（受影响入站接口的汇总，**按 `grpc` / `dubbo` / `http` / `job` 四类分组**）。这条链路**只分析当前这一个服务仓**，不查 BFF；跨仓串联是外部编排层的事。
...
稳定 JSON，有 schema 约束（`schema --type grpc-impact`）。顶层三段 `summary` / `fileSources[]` / `entrySourcesSummary`，其中 `entrySourcesSummary` 的四类分组顺序固定，方便和回归基线对齐。它和 BFF 输出**形状同构、字段定义各自独立**（两者用不同的 schema，投影逻辑不共用）。
