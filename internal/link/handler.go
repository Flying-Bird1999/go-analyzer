...
// ResolveHandlerSymbol 解析 route 的 handler 原始表达式并返回稳定符号 ID。
// 它是 ResolveHandlerSymbolWithConfidence 的薄封装，丢弃置信度信息。ok 为 false 表示无法解析。
...
// ResolveHandlerSymbolWithConfidence 解析 route 的 handler 原始表达式为带置信度的符号。
//
// 解析策略按 "." 分段数决定：
//
//   - 单段（如 "List"）：当前包内同名函数。
...
//
// 返回的置信度反映证据强度：直接命中符号为 high，需经 selector/field 链推断的可能为 medium。
