...
func commonDefinitions() map[string]any {
	return map[string]any{
		...
		"endpoint_asset": object(map[string]any{
			...
			"routes":       arrayOf(ref("dependency_endpoint")),
			"controller":   arrayOf(ref("dependency_symbol")),
			"dependencies": ref("endpoint_asset_dependencies"),
		}, "endpoint", "routes", "controller", "dependencies"),
		"grpc_consumer_impact": object(map[string]any{
			...
			"controller": arrayOf(ref("dependency_symbol")),
			"chains":     arrayOf(ref("dependency_chain")),
		}, "endpoint", "routes", "relation", "controller", "chains"),
		...
		}, "id", "identity", "go_package", "service", "go_method"),
		"grpc_call": object(map[string]any{
			...
			"span":          ref("source_span"),
			"evidence":      arrayOf(ref("evidence")),
		}, "id", "caller_symbol", "operation_id", "span"),
